...
void Context::Render() {
  if (ImGui::Begin("UI WINDOW")) {
    ...
    if (ImGui::CollapsingHeader("light", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      ImGui::DragFloat3("l.direction", glm::value_ptr(m_light.direction),
                        0.01f);
      ImGui::DragFloat2("l.cutoff", glm::value_ptr(m_light.cutoff), 0.1f);
      ImGui::DragFloat("l.distance", &m_light.distance, 1.0f, 1.0f, 500.0f);
  ...
  auto projection = glm::perspective(
      glm::radians(45.0f), (float)m_windowWidth / (float)m_windowHeight, 0.01f,
      20.0f);
  // Set camera
...
bool Context::Init() {
  ...
  std::cout << "simple program id : " << m_simpleProgram->Get() << std::endl;
  ...
  std::cout << "image : " << image->GetWidth() << " * " << image->GetHeight()
            << " " << image->GetChannelCount() << "chanel" << std::endl;

  ImageUPtr image2 = Image::Load("./image/awesomeface.png");
  if (!image2) {
    return false;
  }
  std::cout << "image2 : " << image->GetWidth() << " * " << image->GetHeight()
            << " " << image->GetChannelCount() << "chanel" << std::endl;
